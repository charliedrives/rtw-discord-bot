 ...
 if (interaction.commandName === "rtw_next") {
  ...
  return;
    ...
    if (interaction.commandName === "rtw_post_intro") {
      ...
      const firstMessage = await targetCh.send(RTW2_INTRO_HEADER);

      for (let i = 0; i < RTW2_INTRO_PARAGRAPHS.length; i++) {
        await targetCh.send({ content: RTW2_INTRO_PARAGRAPHS[i], files: [photos[i].url] });
      }

      await firstMessage.pin().catch(() => null);

      await interaction.editReply(`✅ Posted the RTW2 intro (${RTW2_INTRO_PARAGRAPHS.length + 1} messages) to ${targetCh} and pinned the first one.`);
      return;
    }
